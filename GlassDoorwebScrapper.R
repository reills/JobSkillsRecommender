...
#randomly sleep to avoid glassdoor http 429 too many requests
randomSleep <- function(){
  # Generate a random integer between 17 and 27 to sleep for that long and throw off glassdoor timeout
  random_integer <- sample(20:25, 1)
  #sleep randomly between 20-21 seconds to avoid timeout requests
...
saveDataToDB <- function(con, company_html, curr_companyname, curr_industry, curr_url ) {
  ...
  parsedCompanyTable <- parsedCompanyTable %>%
    mutate(company_name = curr_companyname,
           company_industry = curr_industry,
           url = curr_url
    )  
  ...
  #get results of salaries submitted for last row
  lastrow_submitted <-  parsedCompanyTable[nrow(parsedCompanyTable), "salaries_submitted"]
 
  return (lastrow_submitted == "1 Salaries submitted")
}
...
keepRetryingHtmlCaller <- function(url, max_attempts) {
  success <- FALSE
  DELAY_INCREMENT <- 5   #delay is how much to wait in seconds before making another request after getting blocked
  attempts <- 0
  ...
  while (attempts < max_attempts && !success) {
    tryCatch({
      ...
      #copy html so you can process 2 diffferent ways
      explore_html <- company_html
      ...
      element_exists <- !is.null(html_node(explore_html, css = paste("#", "Explore", sep = "")))
...
for(i in 0:GLASSDOOR_COMPANY_LIMIT) {
  ...
  if( i %% GLASSDOOR_COMPANY_PER_PAGE == 0 ) {
    ...
    #random sleep to avoid glassdoor timeouts.
    randomSleep()
  ...
  for(j in 1:companysalary_pages){  
    ...
    #if the last row has only 1 person submitting the salary then it is time to move on - ie scrape the next company
    timeToMoveOn <- saveDataToDB(con, company_html, company_names[company_num],  industry[company_num], next_page_url)
   
    #if salaries submitted = 1 then move on -- the job title is ambigious if only 1 person submitted a salary for it
    if( timeToMoveOn) {
      break
    }
    else {
      #glassdoor only allows so many requests a minute-- sleep before making next request
      randomSleep()
      ...
      company_html <- keepRetryingHtmlCaller( next_page_url, MAX_ATTEMPTS )
    }
  }
}